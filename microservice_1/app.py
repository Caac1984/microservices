...
def pagar():                             # Define a função que será chamada quando uma requisição for feita para o endpoint '/pagar
    ...
    response = requests.post('http://microservice_2:5001/notificar', json=data)    # Faz uma requisição POST para o Microserviço 2, enviando os dados recebidos.
...
if __name__ == '__main__':               # Verifica se este arquivo está sendo executado diretamente.
    app.run(host='0.0.0.0', port=5000)   # Inicia o servidor Flask, tornando a aplicação acessível em todas as interfaces de rede na porta 5000.
